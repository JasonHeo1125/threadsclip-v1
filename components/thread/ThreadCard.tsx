...
export function ThreadCard({ thread, onDelete, onTagClick }: ThreadCardProps) {
  ...
  return (
    <article className="card group overflow-hidden">
      ...
      {thread.memo && (
        <div className="px-4 py-2 bg-[var(--color-bg-elevated)] border-t border-[var(--color-border)] relative z-10">
      ...
      {thread.tags && thread.tags.length > 0 && (
        <div className="flex flex-wrap gap-1 px-4 pb-3 relative z-10 bg-[var(--color-bg-card)]">
          {thread.tags.map((tag) => (
            <button
              key={tag.id}
