...
export function ThreadCard({ thread, onDelete, onTagClick, onUpdateMemo }: ThreadCardProps) {
  ...
  return (
    <>
      <article className="card group overflow-hidden">
        <div className="flex items-center justify-between gap-2 p-4 pb-2">
          ...
          <div className="flex items-center gap-1 flex-shrink-0">
            ...
            <a
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gradient-to-r from-purple-500 to-pink-500 text-white text-xs font-medium hover:from-purple-600 hover:to-pink-600 transition-all shadow-sm hover:shadow-md whitespace-nowrap"
        ...
        {isExpanded && (
          <div className="relative bg-[var(--color-bg-elevated)]">
            ...
            {embedError ? (
              ...
              <div style={{ position: 'relative', overflow: 'hidden', marginBottom: '-70px' }}>
                <iframe
        ...
        <div className="px-4 py-2 pb-4 bg-[var(--color-bg-elevated)] border-t border-[var(--color-border)] relative z-10 group/memo">
          <div className="flex items-start justify-between gap-2">
            <div className="flex items-start gap-2 flex-1">
              ...
              {thread.memo ? (
                <p className="text-sm text-[var(--color-text-secondary)] whitespace-pre-wrap break-words">{thread.memo}</p>
              ) : (
                <p className="text-sm text-[var(--color-text-muted)] italic">메모 없음</p>
              )}
            </div>
            {onUpdateMemo && (
              <button
                onClick={() => setIsEditModalOpen(true)}
                className="opacity-0 group-hover/memo:opacity-100 transition-opacity p-1 hover:bg-[var(--color-border)] rounded text-[var(--color-text-muted)] hover:text-[var(--color-text)]"
                title="메모 수정"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {thread.tags && thread.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 px-4 pt-5 pb-3 relative z-10 bg-[var(--color-bg-card)]">
            {thread.tags.map((tag) => (
              <button
                key={tag.id}
      ...
      <EditMemoModal
        ...
        onSave={async (memo) => {
          if (onUpdateMemo) {
            await onUpdateMemo(thread.id, memo);
          }
        }}
        initialMemo={thread.memo || ''}
      />
    </>
  );
}
